...
export default function TalentFlowDemo() {
  ...
  useEffect(() => {
    ...
    const initializeSDK = async () => {
      ...
      const credentials = await fetchDemoApiKey(sessionId, 'talentflow');

      if (!credentials) {
        console.warn('⚠️ No API key found for TalentFlow demo, continuing without SDK');
        return null;
      }

      const bugspotterSDK = await initializeBugSpotter(
        credentials.apiKey,
